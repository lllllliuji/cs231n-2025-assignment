...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # 2. Word embedding layer: (N, T) -> (N, T, W)
        ...
        # 4. Compute vocabulary scores at each timestep
        scores = temporal_affine_forward(h, W_vocab, b_vocab)  # (N, T, V)

        # 5. Compute loss using temporal softmax
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            # Step 3: embed current word
            word_embed = word_embedding_forward(word, W_embed) # (N, W)

            # Step 4: RNN / LSTM step
            if self.cell_type == "rnn":
                h = rnn_step_forward(word_embed, prev_h, Wx, Wh, b)  # (N, H)
          
            # Step 5: compute vocab scores
            scores = h @ W_vocab + b_vocab

            # Step 6: select most likely next word

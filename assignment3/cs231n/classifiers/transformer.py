...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        # 没有用到transformer里的encoder模块
        # 1. Embed captions and apply positional encoding
        caption_embeddings = self.embedding(captions)  # (N, T, W)
        caption_embeddings = self.positional_encoding(caption_embeddings)  # (N, T, W)

        # 2. Project image features to word vector dimension
        memory = self.visual_projection(features)  # (N, W)
        memory = memory.unsqueeze(1)  # (N, 1, W) -- acts as one-step "encoder" output

        # 3. Create causal mask for decoder，上三角全为0
        tgt_mask = torch.tril(torch.ones((T, T), device=captions.device)).bool()

        # 4. Decode caption embeddings using Transformer decoder
        out = self.transformer(caption_embeddings, memory, tgt_mask=tgt_mask)  # (N, T, W)

        # 5. Project to vocabulary size to get scores
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.positional_encoding(patches)  # (N, num_patches, embed_dim)

        # Step 3: Transformer Encoder
        out = self.transformer(out)  # (N, num_patches, embed_dim)

        # Step 4: Global Average Pooling over patches
        out = out.mean(dim=1)  # (N, embed_dim)

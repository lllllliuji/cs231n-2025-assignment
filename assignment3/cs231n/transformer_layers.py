...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, 0::2] = torch.sin(position * div_term)  # 偶数索引位置
        pe[0, :, 1::2] = torch.cos(position * div_term)  # 奇数索引位置
    ...
    def forward(self, x):
        ...
        ############################################################################
        output = self.dropout(x + self.pe[:, :S, :])  # (N, S, D)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return output
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # 拆成多头并调整维度顺序: (N, S or T, E) -> (N, H, S or T, D)
        def reshape_heads(x):
            return x.reshape(N, -1, self.n_head, self.head_dim).transpose(1, 2)

        Q = reshape_heads(Q)   # (N, H, S, D)
        K = reshape_heads(K)   # (N, H, T, D)
        V = reshape_heads(V)   # (N, H, T, D)

        # 计算注意力打分: Q @ K^T -> (N, H, S, T)
        scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.head_dim ** 0.5)  # (N, H, S, T)

        # 应用 mask（如果提供）
        if attn_mask is not None:
            # attn_mask: (S, T), broadcast to (1, 1, S, T)
            scores = scores.masked_fill(attn_mask == 0, float('-inf'))

        # 注意力权重: softmax 后表示注意力分配概率
        attn = torch.softmax(scores, dim=-1)  # (N, H, S, T)
        attn = self.attn_drop(attn)          # (N, H, S, T)

        # 计算加权和：注意力乘上值向量 V
        context = torch.matmul(attn, V)      # (N, H, S, D)

        # 合并多头：先转置再 reshape 回原始维度
        context = context.transpose(1, 2).reshape(N, S, E)  # (N, S, E)

        output = self.proj(context)          # (N, S, E)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return output
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # Step 1: Split image into blocks
        x = x.view(N, C, H // self.patch_size, self.patch_size,
                  W // self.patch_size, self.patch_size)

        # Step 2: Permute to get (N, H//p, W//p, p, p, C)
        x = x.permute(0, 2, 4, 3, 5, 1)

        # Step 3: Flatten each patch into vector of shape (patch_size * patch_size * C)
        patches = x.reshape(N, -1, self.patch_dim)

        # Step 4: Apply linear projection

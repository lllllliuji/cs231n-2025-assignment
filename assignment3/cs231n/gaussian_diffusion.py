...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # Step 2: Determine if the model outputs noise or x_start
        ...
        # Step 3: Clamp predicted x_start to valid range [-1, 1]
        x_start.clamp_(-1.0, 1.0)

        # Step 4: Get posterior mean and std
        posterior_mean, posterior_std = self.q_posterior(x_start=x_start, x_t=x_t, t=t)

        # Sample x_{t-1} from the posterior distribution
        if t[0] > 0:  # Only add noise if not at the last step
            noise = torch.randn_like(x_t)
            x_tm1 = posterior_mean + posterior_std * noise
        else:
            x_tm1 = posterior_mean  # No noise added at the final step
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        loss = (loss_weight * torch.nn.functional.mse_loss(model_output, target, reduction='none')).mean()
        ####################################################################

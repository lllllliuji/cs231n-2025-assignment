...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    text_norm = nn.functional.normalize(text_features, dim=1)   # (N, D)
    image_norm = nn.functional.normalize(image_features, dim=1) # (M, D)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    # 1. 将文本类别转换为 token，并编码为特征向量
    text_tokens = clip.tokenize(class_texts).to(device)  # (C, T)
    text_features = clip_model.encode_text(text_tokens)  # (C, D)
    text_features = nn.functional.normalize(text_features, dim=1)  # 单位化

    # 2. 预处理并编码所有图像
    image_inputs = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)  # (N, 3, H, W)
    image_features = clip_model.encode_image(image_inputs)  # (N, D)
    image_features = nn.functional.normalize(image_features, dim=1)  # 单位化

    # 3. 计算相似度矩阵
    similarity = get_similarity_no_loop(text_features, image_features)  # (C, N)
    similarity = similarity.T  # (N, C)

    # 4. 为每张图像选择最高相似度对应的类别
    pred_indices = similarity.argmax(dim=1)  # (N,)
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        ]).to(device)  # Shape: (N, 3, H, W)

        # 2. 提取图像特征并归一化
        self.image_features = clip_model.encode_image(image_tensors)  # (N, D)
        self.image_features = nn.functional.normalize(self.image_features, dim=1)  # 单位化向量
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        ############################################################################
        # 1. 编码并归一化查询文本
        text_tokens = clip.tokenize([query]).to(self.device)  # Shape: (1, T)
        text_features = self.clip_model.encode_text(text_tokens)  # (1, D)
        text_features = nn.functional.normalize(text_features, dim=1)  # 单位化

        # 2. 计算与图像的余弦相似度
        similarity = text_features @ self.image_features.T  # Shape: (1, N)

        # 3. 获取 top-k 最大相似度的图像索引
        topk_values, topk_indices = similarity.topk(k, dim=1)  # Shape: (1, k)

        return topk_indices.squeeze(0).tolist()  # 转为 List[int]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        # Simple two-layer neural network (can be replaced with nn.Linear for a linear model)

...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ...
            up_block = nn.ModuleList([
                    ...
                    ResnetBlock(dim_out * 2, dim_out, context_dim=context_dim),  # skip connection: concat
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        # Unconditional prediction (remove condition)
        model_kwargs["text_emb"] = None
        uncond_pred = self.forward(x, time, model_kwargs)

        # Classifier-free guidance equation
        x = (1 + cfg_scale) * cond_pred - cfg_scale * uncond_pred
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for upsample, resnet1, resnet2 in self.ups:
            x = upsample(x)
            x = torch.cat((x, skip_connections.pop()), dim=1)  # 1st skip conn
            x = resnet1(x, context)
            x = torch.cat((x, skip_connections.pop()), dim=1)  # 2nd skip conn
            x = resnet2(x, context)
        ##################################################################
